...
st.title("📊 Evaluation Logs")
st.caption("Langfuse に格納済みの評価スコアと会話ログを表示します。")
...
with st.sidebar:
    ...
    from_date = st.date_input("開始日", value=today - timedelta(days=1))
    to_date = st.date_input("終了日", value=today)

    limit = st.slider("取得上限 (件)", min_value=100, max_value=2000, value=500, step=100)

    host = st.text_input(
        "Langfuse ホスト",
        value=os.environ.get("LANGFUSE_HOST", "https://us.cloud.langfuse.com"),
        help="セルフホストの場合は変更してください。",
    )

    refresh = st.button("データ更新", type="primary", width="stretch")
...
if df.empty:
    st.warning("指定した期間にスコアデータが見つかりませんでした。期間やフィルタを変更してください。")
    st.stop()
...
col1.metric("総スコア件数", len(df_filtered))
col2.metric("対象トレース数", df_filtered["trace_id"].nunique())
col3.metric("評価観点数", df_filtered["score_name"].nunique())
...
st.subheader("時系列スコア推移")

numeric_df = df_filtered[df_filtered["data_type"] == "NUMERIC"].copy()

if numeric_df.empty:
    st.info("数値スコアが見つかりませんでした。Categorical スコアは下のテーブルで確認できます。")
else:
    fig = go.Figure()
    ...
    with st.expander(label):
        ...
        for _, row in group.iterrows():
            score_rows.append(
                {
                    "評価観点": row["score_name"],
                    "スコア": row["score_value"] if row["score_value"] is not None else row["string_value"],
                    "種別": row["data_type"],
                    "コメント": row["comment"],
                }
            )
